"""
 ...
 Class for ORM model describing job for getting devices from target
...
class GetDevicesJobModel(JobsModel):
    __tablename__ = 'get_devices_jobs'

    __mapper_args__ = {
        'polymorphic_identity': JobTypesEnum.get_devices_type.value
    }
    ...
    target: RemoteTargetModel = relationship('RemoteTargetModel', foreign_keys=[target_id],
                                             backref=backref('get_devices_job',
                                                             lazy='subquery',
                                                             cascade='delete,all'))
